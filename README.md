...
Registers an ESM namespace object or module redirect and returns `Promise<void>`. With `{ lazy: true, exports: [...] }`, `replacement` is a synchronous factory and `exports` declares the names Node must link before evaluating it. See [Native ESM](#native-esm) for loader requirements.
