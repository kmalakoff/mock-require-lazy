...
export function add(url: string, value: unknown, options: ImportOptions, ticket: Ticket): void {
  ...
  if (!replace(url, { id, names: unique, control }, ticket)) records.delete(id);
}

export function redirect(url: string, target: string, ticket: Ticket): void {
