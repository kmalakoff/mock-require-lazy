...
if (Number(process.versions.node.split('.')[0]) >= 12) {
  describe('ESM registration state', () => {
    ...
    it('reports the assigned id from add, or undefined when a newer registration already won', () => {
      const older = state.begin('./dep.mjs', caller);
      const newer = state.begin('dependency-alias', caller);
      const id = state.add(target, { value: 'newer' }, {}, newer);
      assert.strictEqual(typeof id, 'number');
      state.finish(newer);
      assert.strictEqual(state.add(target, { value: 'older' }, {}, older), undefined);
      state.finish(older);
    });

    it('reports whether redirect took effect, false when a newer registration already won', () => {
      const older = state.begin('./dep.mjs', caller);
      const newer = state.begin('dependency-alias', caller);
      const other = 'file:///mock-tests/other.mjs';
      assert.strictEqual(state.redirect(other, target, newer), true);
      state.finish(newer);
      assert.strictEqual(state.redirect(other, target, older), false);
      state.finish(older);
    });
