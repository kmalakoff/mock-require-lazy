...
export default async function run(mock) {
  ...
  // One mode: 'both' registration replaces the package's require and import conditions together.
  await mock('mrl-conditional-fixture', { value: 'both-fake' }, { mode: 'both' });
  assert.strictEqual(require('mrl-conditional-fixture').value, 'both-fake');
  assert.strictEqual((await import('mrl-conditional-fixture')).value, 'both-fake');
  mock.stop('mrl-conditional-fixture');
  assert.strictEqual(require('mrl-conditional-fixture').value, 'require-real');
  assert.strictEqual((await import('mrl-conditional-fixture')).value, 'import-real');
}
